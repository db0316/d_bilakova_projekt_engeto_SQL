WITH avg_payroll_max_date AS 
	(SELECT 
		round(avg(dbprimar.average_wages)::NUMERIC, 2) AS avg_payroll_max_date -- pro zjednoduseni avg za vsechna odvetvi (nemam k dispozici hodnotu NULL)
	FROM 
...
avg_payroll_min_date AS
	(SELECT 
		round(avg(dbprimar.average_wages)::NUMERIC, 2) AS avg_payroll_min_date -- pro zjednoduseni avg za vsechna odvetvi (nemam k dispozici hodnotu NULL)
	FROM 
		t_Daniela_Bilakova_project_SQL_primary_final AS dbprimar
	WHERE 
		payroll_year IN 
...
SELECT 
	...
	, round((avgmax.avg_payroll_max_date / fcmin.price_min_date)::NUMERIC, 2) AS pocet_max_date 
FROM 
